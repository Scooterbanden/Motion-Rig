...
// Struct instances
servo_t servo[] = {
    {false, {GPIOD, S1_Ready_Pin}, false, {GPIOD, S1_Treach_Pin}, false, {GPIOD, S1_Enable_Pin}, {GPIOD, S1_Direction_Pin}, 0, &htim8, TIM_CHANNEL_4, HRTIM_TIMERINDEX_TIMER_C, {&htim3, 0, {GPIOD, S1_EncZ_Pin}, 0, 0}, false, {NULL, 0, 0}, 0},
	{false, {GPIOA, S2_Ready_Pin}, false, {GPIOA, S2_Treach_Pin}, false, {GPIOD, S2_Enable_Pin}, {GPIOC, S2_Direction_Pin}, 0, &htim12, TIM_CHANNEL_1, HRTIM_TIMERINDEX_TIMER_B, {&htim2, 0, {GPIOC, S2_EncZ_Pin}, 0, 0}, false, {&htim1, 0, 0}, 0},
...
void servoInit(void) {
	...
	for (int i = 0; i < 4; i++) {
		if ((getGPIO(servo[i].readyPin) == GPIO_PIN_RESET)) {
			...
			if (i != 2) {
				HAL_TIM_Encoder_Start(servo[i].pulseCounter.timer, TIM_CHANNEL_ALL);
			} else {
